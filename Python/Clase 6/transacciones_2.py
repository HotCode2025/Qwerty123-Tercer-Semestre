...
try:
        #Ejemplo de una transacción que incluye una inserción y una actualización
    conexion.autocommit = False #Desactivamos el autocommit para manejar las transacciones manualmente
    cursor = conexion.cursor()
    #Primera transaccion: Insertar un nuevo registro en la tabla persona
    sentencia = "INSERT INTO persona(nombre, apellido, email) VALUES (%s, %s, %s)"
    valores = ("Carlos", "Lara", "carlos.lara@email.com")
    ...
    valores = ("Juan","Juarez", "juan.juarez@email.com",1)
    cursor.execute(sentencia, valores)
    print("Registro actualizado, pero aún no se ha confirmado la transacción")
